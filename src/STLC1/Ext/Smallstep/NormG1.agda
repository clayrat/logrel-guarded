{-# OPTIONS --guarded #-}
module STLC1.Ext.Smallstep.NormG1 where
...
R⇒ : ∀ {T₁ T₂ t}
   ...
   → (∀ s → ▹ R T₁ s → Part (R T₂ (t · s)))
   → R (T₁ ⇒ T₂) t
R⇒ {T₁} {T₂} {t} ⊢t h r = ⇉R $ R⇒F ⊢t h λ s x → mapᵖ R⇉ $ r s x
...
... | R⇒F ⊢t h r = ⊢t , h , λ s x → mapᵖ ⇉R (r s x)
...
step-preserves-R {T = T₁ ⇒ T₂} S r = let tp , h , Ri = R⇒-match r in
  R⇒ (preserve tp S) (step-preserves-halting S .fst h)
     (λ t″ R₁ → mapᵖ (step-preserves-R (ξ-·₁ S)) (Ri t″ R₁))
...
step-preserves-R' {T = T₁ ⇒ T₂} {t} {t′} tp S r = let _ , h′ , Ri = R⇒-match r in
  R⇒ tp (step-preserves-halting S .snd h′)
     λ t″ R▹ → later $ ▹map (λ R″ → mapᵖ (step-preserves-R' (tp ⊢· R-typable-empty R″) (ξ-·₁ S)) (Ri t″ R▹)) R▹
  ...
  now $ R⇒ (subst (λ q → ∅ ⊢ q ⦂ A ⇒ B) (sym mabs) WT)
           ...
           (λ s → later ∘ ▹map λ Rs →
                     ...
                     mapᵖ (subst (λ q → R B (q · s)) (sym mabs) ∘
                            multistep-preserves-R' (WT ⊢· R-typable-empty Rs)
  ...
  bothᵖ (msubst-R ⊢L i) (msubst-R ⊢M i) >>=ᵖ λ where
    (R1 , R2) →
      let (_ , _ , R1→) = R⇒-match R1
       ...
       in
      subst (λ q → Part (R T q)) (sym $ msubst-app e L M) Rapp
